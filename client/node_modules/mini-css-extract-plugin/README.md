...
See [examples](#remove-orders-warnings) below for details.
...
module.exports = {
  plugins: [
    new MiniCssExtractPlugin({
      ...
      filename: "[name].css",
      chunkFilename: "[id].css",
      ignoreOrder: false, // Enable to remove warnings about conflicting orders
    }),
  ],
  module: {
    rules: [
...
For projects where css ordering has been mitigated through consistent use of scoping or naming conventions, such as [CSS Modules](https://github.com/css-modules/css-modules), the css orders warnings can be disabled by setting the ignoreOrder flag to true for the plugin.
