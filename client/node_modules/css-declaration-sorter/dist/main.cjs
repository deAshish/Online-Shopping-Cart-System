...
const cssDeclarationSorter = ({ orders = 'alphabetical', keepOverrides = false } = {}) => ({
  ...
  OnceExit (css) {
    ...
    if (typeof orders === 'function') {
      return processCss({ css, comparator: withKeepOverrides(orders) });
    }

    if (!builtInOrders.includes(orders))
      return Promise.reject(
        Error([
          `Invalid built-in orders '${orders}' provided.`,
          `Available built-in orders are: ${builtInOrders}`,
        ].join('\n'))
      );

    return __variableDynamicImportRuntime0__(`../orders/${orders}.mjs`)
...
function orderComparator (orders) {
  return function (a, b) {
