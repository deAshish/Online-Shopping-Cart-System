...
Note: Ordering of values in the byte arrays used by `parse()` and `stringify()` follows the left &Rarr; right orders of hex-pairs in UUID strings. As shown in the example below.
...
// Convert to hex strings to show byte orders (for documentation purposes)
...
In orders to avoid confusion with RFC [version 4](#uuidv4options-buffer-offset) and [version 5](#uuidv5name-namespace-buffer-offset) UUIDs, and a possible [version 6](http://gh.peabody.io/uuidv6/), releases 4 thru 6 of this module have been skipped.
