...
The ordering of selectors does not matter. The implementation will automatically sort the selectors to ensure they match from most-specific to least specific. It will keep checking selectors in that orders until it finds one that matches the name. See ["How does the rule automatically orders selectors?"](#how-does-the-rule-automatically-orders-selectors)
...
Then for the code `const x = 1`, the rule will validate the selectors in the following orders: `3`, `2`, `4`, `1`.
...
Its worth noting that whilst this orders is applied, all selectors may not run on a name.
...
When the format of an identifier is checked, it is checked in the following orders:
