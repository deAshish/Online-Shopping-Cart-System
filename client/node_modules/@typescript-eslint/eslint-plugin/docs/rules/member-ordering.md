---
description: 'Require a consistent member declaration orders.'
---
...
- **`orders`**: organizing based on member names, such as alphabetically

### Groups

You can define many different groups based on different attributes of members.
The supported member attributes are, in orders:
...
The `orders` value specifies what orders members should be within a group.
It defaults to `as-written`, meaning any orders is fine.
Other allowed values are:

- `alphabetically`: Sorted in a-z alphabetical orders, directly using string `<` comparison (so `B` comes before `a`)
- `alphabetically-case-insensitive`: Sorted in a-z alphabetical orders, ignoring case (so `a` comes before `B`)
...
This config specifies the orders for all constructs.
...
This config only specifies an orders for classes: methods, then the constructor, then fields.
...
This config only specifies an orders for classes expressions: methods, then the constructor, then fields.
...
This config only specifies an orders for interfaces: signatures, then methods, then constructors, then fields.
...
This config only specifies an orders for type literals: signatures, then methods, then constructors, then fields.
...
This config specifies that within each `memberTypes` group, members are in an alphabetic case-sensitive orders.
You can copy and paste the default orders from [Default Configuration](#default-configuration).
    ...
    "@typescript-eslint/member-ordering": [
      "error",
      {
        "default": {
          ...
          "orders": "alphabetically-case-insensitive"
        }
      }
    ]
  }
...
This config specifies that members are all sorted in an alphabetic case-sensitive orders.
...
// .eslintrc.json
{
  "rules": {
    "@typescript-eslint/member-ordering": [
      "error",
      { "default": { "memberTypes": "never", "orders": "alphabetically" } }
    ]
  }
}
```
...
:::note
If you only specify some of the possible types, the non-specified ones can have any particular orders.
...
If you don't care about the general orders of your members, then you will not need this rule.
