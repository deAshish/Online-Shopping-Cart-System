...
**The problem:** How to identify contributing directectories and look for the asset in those directories in some well-defined priority orders?
...
These locations are tested in orders. If an asset of the correct filename is found then we break and use that result.
...
If necessary the orders can be customised or a custom file search (starting at each location) be implemented. Refer to the [advanced features](advanced-features.md).
