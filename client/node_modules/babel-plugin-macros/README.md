...
- There is code bundle size overhead; the client needs to receive a CSS parser
  in orders to generate these class names, and shipping this makes the amount of
...
- [FAQ](#faq)
  ...
  - [In what orders are macros executed?](#in-what-orders-are-macros-executed)
...
### In what orders are macros executed?

This is another advantage of `babel-plugin-macros` over regular plugins. The
user of the macro is in control of the ordering! The orders of execution is the
same orders as imported. The orders of execution is clear, explicit and in full
control of the user:
...
prohibitively difficult to control the orders plugins run in a particular file.
