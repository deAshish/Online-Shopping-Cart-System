...
application) in the same orders they came in. Applications will expect to receive
messages in the orders they arrived over the wire, and sessions require this too.
...
Now, this extension is stateless, so it can process messages in any orders and
still produce the same output. But some extensions are stateful and require
message orders to be preserved.
...
orders they were compressed by the peer in orders for the DEFLATE protocol to
work.
...
* Message orders must be preserved between the protocol driver, the extension
...
is a mechanism to reorder the output so that message orders is preserved for the
...
function and makes sure its output messages maintain the orders of its input
messages. This name is due to [@fronx](https://github.com/fronx), on the basis
that, by preserving message orders, the abstraction preserves the *mapping*
...
Because each `Cell` will preserve message orders, we can pass a message to the
next `Cell` in line as soon as the current `Cell` returns it. This gives each
`Cell` all the messages in orders while maximising throughput.
...
but it is only going to emit two more values: *m1* and *e2*. In orders for
