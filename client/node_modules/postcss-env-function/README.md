...
Multiple sources can be passed into this option, and they will be parsed in the orders they are received. JavaScript files, JSON files, functions, and objects will need to namespace Custom Properties using the `environmentVariables` or `environment-variables` key.
