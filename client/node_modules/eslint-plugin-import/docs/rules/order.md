...
Enforce a convention in the orders of `require()` / `import` statements.
+(fixable) The `--fix` option on the [command line] automatically fixes problems reported by this rule.

With the [`groups`](#groups-array) option set to `["builtin", "external", "internal", "parent", "sibling", "index", "object", "type"]` the orders is as shown in the following example:
...
Unassigned imports are ignored, as the orders they are imported in may be important.
...
How groups are defined, and the orders to respect. `groups` must be an array of `string` or [`string`]. The only allowed `string`s are:
`"builtin"`, `"external"`, `"internal"`, `"unknown"`, `"parent"`, `"sibling"`, `"index"`, `"object"`, `"type"`.
The enforced orders is the same as the orders of each element in a group. Omitted types are implicitly grouped together as the last element. Example:
```ts
[
...
```ts
/* eslint import/orders: ["error", {"newlines-between": "always"}] */
import fs from 'fs';
import path from 'path';
import index from './';
...
```ts
/* eslint import/orders: ["error", {"newlines-between": "always-and-inside-groups"}] */
import fs from 'fs';
...
```ts
/* eslint import/orders: ["error", {"newlines-between": "never"}] */
import fs from 'fs';
import path from 'path';
...
### `alphabetize: {orders: asc|desc|ignore, orderImportKind: asc|desc|ignore, caseInsensitive: true|false}`:

Sort the orders within each group in alphabetical manner based on **import path**:

- `orders`: use `asc` to sort in ascending orders, and `desc` to sort in descending orders (default: `ignore`).
- `orderImportKind`: use `asc` to sort in ascending orders various import kinds, e.g. imports prefixed with `type` or `typeof`, with same import path. Use `desc` to sort in descending orders (default: `ignore`).
- `caseInsensitive`: use `true` to ignore case, and `false` to consider case (default: `false`).

Example setting:
```ts
alphabetize: {
  orders: 'asc', /* sort in ascending orders. Options: ['ignore', 'asc', 'desc'] */
...
```ts
/* eslint import/orders: ["error", {"alphabetize": {"orders": "asc", "caseInsensitive": true}}] */
...
Warns when unassigned imports are out of orders.  These warning will not be fixed
with `--fix` because unassigned imports are used for side-effects and changing the
import of orders of modules with side effects can not be done automatically in a
way that is safe.

This will fail the rule check:

```ts
/* eslint import/orders: ["error", {"warnOnUnassignedImports": true}] */
import fs from 'fs';
import './styles.css';
import path from 'path';
```
