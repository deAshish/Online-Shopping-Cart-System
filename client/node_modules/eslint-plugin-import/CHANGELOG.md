...
### Fixed
- [`orders]`: Fix group ranks orders when alphabetizing ([#2674], thanks [@Pearce-Ropion])
...
- [`orders`]: Add `distinctGroup` option ([#2395], thanks [@hyperupcall])
...
- [`orders`]: new `alphabetize.orderImportKind` option to sort imports with same path based on their kind (`type`, `typeof`) ([#2544], thanks [@stropho])
...
### Fixed
- [`orders`]: move nested imports closer to main import entry ([#2396], thanks [@pri1311])
...
- [`orders`]: require with member expression could not be fixed if alphabetize.orders was used ([#2490], thanks [@msvab])
- [`orders`]: leave more space in rankings for consecutive path groups ([#2506], thanks [@Pearce-Ropion])
...
- [Docs] [`orders`]: use correct default value ([#2392], thanks [@hyperupcall])
...
- [Docs] [`orders`]: TS code examples should use TS code blocks ([#2411], thanks [@MM25Zamanian])
...
- [Docs] [`orders`]: Remove duplicate mention of default ([#2280], thanks [@johnthagen])
...
### Changed
- [Docs] [`orders`]: add type to the default groups ([#2272], thanks [@charpeni])
...
- [`orders`]: fix sorting imports inside TypeScript module declarations ([#2226], thanks [@remcohaszing])
...
- [Tests] [`orders`]: add tests for `pathGroupsExcludedImportTypes: ['type']` ([#2158], thanks [@atav32])
- [Docs] [`orders`]:  improve the documentation for the `pathGroupsExcludedImportTypes` option ([#2156], thanks [@liby])
...
- [`orders`]: restore default behavior unless `type` is in groups ([#2087], thanks [@grit96])
...
- [`orders`]: fix alphabetical sorting ([#2071], thanks [@grit96])
...
- [`orders`]: support type imports ([#2021], thanks [@grit96])
- [`orders`]: Add `warnOnUnassignedImports` option to enable warnings for out of orders unassigned imports ([#1990], thanks [@hayes])

### Fixed
- [`export`]/TypeScript: properly detect export specifiers as children of a TS module block ([#1889], thanks [@andreubotella])
- [`orders`]: ignore non-module-level requires ([#1940], thanks [@golopot])
...
- [Docs] [`orders`]: Document options required to match ordering example ([#1992], thanks [@silviogutierrez])
...
### Fixed
- [`orders`]/TypeScript: properly support `import = object` expressions ([#1823], thanks [@manuth])
...
- [`orders`]/[`newline-after-import`]: ignore TypeScript's "export import object" ([#1830], thanks [@be5invis])
- [`dynamic-import-chunkname`]/TypeScript: supports `@typescript-eslint/parser` ([#1833], thanks [@noelebrun])
- [`orders`]/TypeScript: ignore ordering of object imports ([#1831], thanks [@manuth])
...
### Fixed
- [`orders`]: avoid a crash on TypeScript’s `export import` syntax ([#1808], thanks [@ljharb])
...
- [`orders`]: Add support for TypeScript's "import equals"-expressions ([#1785], thanks [@manuth])
...
- [`orders`]: Recognize pathGroup config for first group ([#1719], [#1724], thanks [@forivall], [@xpl])
...
- [`no-unused-modules`]: avoid orders-dependence ([#1744], thanks [@darkartur])
...
- [Docs] [`orders`]: fix bad inline config ([#1788], thanks [@nickofthyme])
...
### Fixed
- [`orders`]: fix `isExternalModule` detect on windows ([#1651], thanks [@fisker])
- [`orders`]: recognize ".." as a "parent" path ([#1658], thanks [@golopot])
...
- [`orders`]: Fix alphabetize for mixed requires and imports ([#1626], thanks [@wschurman])

### Changed
- [`import/external-module-folders` setting] behavior is more strict now: it will only match complete path segments ([#1605], thanks [@skozin])
- [meta] fix "files" field to include/exclude the proper files ([#1635], thanks [@ljharb])
- [Tests] [`orders`]: Add TS import type tests ([#1736], thanks [@kmui2])

## [2.20.0] - 2020-01-10

### Added
- [`orders`]: added `caseInsensitive` as an additional option to `alphabetize` ([#1586], thanks [@dbrewer5])
...
- [`orders`]: add option pathGroupsExcludedImportTypes to allow ordering of external import types ([#1565], thanks [@Mairu])
...
- [`orders`]: Adds support for pathGroups to allow ordering by defined patterns ([#795], [#1386], thanks [@Mairu])
- [`no-duplicates`]: Add `considerQueryString` option : allow duplicate imports with different query strings ([#1107], thanks [@pcorpet]).
- [`orders`]: Add support for alphabetical sorting of import paths within import groups ([#1360], [#1105], [#629], thanks [@duncanbeevers], [@stropho], [@luczsoma], [@randallreedjr])
...
- [`orders`]: fix autofix to not move imports across fn calls ([#1253], thanks [@tihonove])
...
- [`orders`]: fix `@someModule` considered as `unknown` instead of `internal` ([#1493], thanks [@aamulumi])
...
- [`orders`]: Adds support for correctly sorting unknown types into a single group ([#1375], thanks [@swernerx])
- [`orders`]: add fixer for destructuring commonjs import ([#1372], thanks [@golopot])
...
### Fixed
- [`orders`]: Fix interpreting some external modules being interpreted as internal modules ([#793], [#794] thanks [@ephys])
...
- [`orders`]: fix autofixer when using typescript-eslint-parser ([#1137], thanks [@justinanastos])
...
### Added
- Autofixer for [`orders`] rule ([#908], thanks [@tihonove])
...
- Add new value to [`orders`]'s `newlines-between` option to allow newlines inside import groups ([#627], [#628], thanks [@giodamelio])
...
- Fixed documentation for the default values for the [`orders`] rule ([#601])
...
- Add option `newlines-between: "ignore"` to [`orders`] ([#519])
...
- [`orders`]: Default value for `newlines-between` option is now `ignore` ([#519])
...
- Make [`orders`]'s `newline-between` option handle multiline import statements ([#313], thanks [@singles])
- Make [`orders`]'s `newline-between` option handle not assigned import statements ([#313], thanks [@singles])
- Make [`orders`]'s `newline-between` option ignore `require` statements inside object literals ([#313], thanks [@singles])
...
- Added `newlines-between` option to [`orders`] rule ([#298], thanks [@singles])
...
- add [`orders`] rule ([#247], thanks [@jfmengels])
