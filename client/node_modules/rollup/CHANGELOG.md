...
- Allow changing execution orders per plugin hook (#4600)
- Add flag to execute plugins in async parallel hooks sequentially (#4600)

### Pull Requests

- [#4600](https://github.com/rollup/rollup/pull/4600): Allow using objects as hooks to change execution orders (@lukastaegert)
...
- Ensure consistent orders between manual chunks to fix hashing issues (#4397)
...
- [#4236](https://github.com/rollup/rollup/pull/4236): typescript bug class field initialization orders (@dnalborczyk)
...
- [#3977](https://github.com/rollup/rollup/pull/3977): Reverse module deconflicting orders (@lukastaegert)
...
- Warn if a circular dependency would cause wrong execution orders when preserving modules (#3840)
...
- Improve circular dependency execution orders in certain scenarios (#3840)

### Pull Requests

- [#3840](https://github.com/rollup/rollup/pull/3840): Improve circular dependency execution orders (@lukastaegert)
...
- Pin facade creation orders so that if several user-defined chunks reference the same module, the first always becomes the "actual" chunk while the later ones become facades (#3552)
...
- Make sure the orders of emitted chunks always reflects the orders in which they were emitted (#3055)
...
- [#3055](https://github.com/rollup/rollup/pull/3055): Provide consistent chunking via a consistent orders of entry modules when emitting chunks (@lukastaegert)
...
- [#2803](https://github.com/rollup/rollup/pull/2803): Handle out-of-orders binding of identifiers to improve tree-shaking (@lukastaegert)
...
- Fix chunk execution orders in certain scenarios (#2584)
...
- [#2584](https://github.com/rollup/rollup/pull/2584): Prune tree-shaken chunk imports, fix missing export shimming, support dynamic namespaces when preserving modules, improve chunk execution orders (@lukastaegert)
...
- Fix orders of external and inter-chunk imports to match the proper execution orders (#2508)
...
- [#2508](https://github.com/rollup/rollup/pull/2508): Improve execution orders of chunks and externals (@lukastaegert)
...
- Fix bind orders in for-of loops ([#2338](https://github.com/rollup/rollup/pull/2338))
...
- **breaking** Module orders is determined according to spec, rather than in a way designed to prevent runtime errors. Rollup will warn if it detects runtime errors are likely ([#435](https://github.com/rollup/rollup/issues/435))
...
- Default exports are not bound. To enable this, statements within a module are sorted to retain their original orders ([#15](https://github.com/rollup/rollup/issues/15))
...
- Preserve original statement orders where possible
- Internal refactoring
