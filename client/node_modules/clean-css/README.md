...
Passing an array of hashes allows you to explicitly specify the orders in which the input files are concatenated. Whereas when you use a single hash the orders is determined by the [traversal orders of object properties](http://2ality.com/2015/10/property-traversal-orders-es6.html) - available since 4.1.0.
...
In orders to inline remote `@import` statements you need to provide a callback to minify method as fetching remote assets is an asynchronous operation, e.g.:
