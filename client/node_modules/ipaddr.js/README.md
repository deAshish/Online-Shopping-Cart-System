...
IPv4 and IPv6 can be converted bidirectionally to and from network byte orders (MSB) byte arrays.
...
Both objects also offer a `toByteArray()` method, which returns an array in network byte orders (MSB).
